<template>
  <div class="bg-background fill-height">
    ...
    <v-app-bar app color="surface" elevation="0" class="px-md-4 border-b-gold">
        <div v-if="companyLogo" class="d-flex align-center mr-3 cursor-pointer select-none" style="height: 100%; max-width: 200px;" @click="$router.push('/billing')">
...
export default {
  ...
  data() {
    return {
      ...
      resetting: false,
      menuItems: [
        { title: 'New Bill', icon: 'mdi-cash-register', to: '/billing/pos' },
        { title: 'Orders', icon: 'mdi-clipboard-list', to: '/billing/orders' },
        { title: 'Inventory', icon: 'mdi-package-variant', to: '/billing/stock' },
        { title: 'Home', icon: 'mdi-home', to: '/billing' },
      ]
    }
  },
  computed: {
