<template>
  <v-container fluid class="fill-height bg-grey-lighten-5 pa-6 d-flex flex-column align-start">
    ...
    <v-card class="w-100 rounded-xl elevation-0 border-thin bg-white pa-8 mt-6" style="max-width: 600px; margin: 0 auto">
       <div class="d-flex align-center mb-6">
           <div class="bg-amber-lighten-5 pa-4 rounded-circle mr-4">
               <v-icon color="amber-darken-2" size="32">mdi-layers-triple</v-icon>
           </div>
           <div>
               <div class="text-h6 font-weight-bold text-grey-darken-3">Tiered Rules</div>
       ...
       <!-- Add Rule Form -->
       <div class="bg-grey-lighten-5 pa-4 rounded-lg mb-6 border-thin">
           <div class="text-caption font-weight-bold text-uppercase text-grey-darken-1 mb-2">Add New Rule</div>
           <v-row dense>
               <v-col cols="4">
                   <v-text-field v-model.number="newRule.min" label="Min Amount" variant="outlined" density="compact" bg-color="white" hide-details type="number"></v-text-field>
               </v-col>
               <v-col cols="4">
                   <v-text-field v-model.number="newRule.max" label="Max Amount" variant="outlined" density="compact" bg-color="white" hide-details type="number" placeholder="0 for Infinity"></v-text-field>
               </v-col>
               <v-col cols="4">
                   <v-text-field v-model.number="newRule.percent" label="Discount %" variant="outlined" density="compact" bg-color="white" hide-details type="number" suffix="%"></v-text-field>
               </v-col>
           </v-row>
           <v-btn block color="amber-darken-2" class="mt-3 text-white" rounded="lg" @click="addRule" :loading="addingRule" :disabled="!isValidRule">
               Add Rule
           </v-btn>
       </div>
       ...
       <v-list v-else bg-color="transparent">
           <v-list-item v-for="rule in rules" :key="rule.id" class="mb-2 rounded-lg border bg-white elevation-1">
               <template v-slot:prepend>
                   <v-avatar color="amber-lighten-4" class="text-amber-darken-3 font-weight-bold">{{ rule.percentage }}%</v-avatar>
               </template>
