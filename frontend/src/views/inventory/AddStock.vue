  ...
  <v-container fluid class="fill-height bg-background pa-6 d-flex justify-center align-center">
    <v-card width="600" class="rounded-xl elevation-10 modern-card bg-surface border-thin overflow-visible">
      ...
      <v-toolbar color="transparent" class="px-6 pt-6 mb-2">
          ...
          <div>
             <v-toolbar-title class="text-h5 font-weight-black gradient-text">Stock Entry</v-toolbar-title>
      ...
      <v-card-text class="pa-6 pt-2">
        ...
        <v-expand-transition>
            <div v-if="selectedProduct" class="mt-2">
                <v-card class="bg-surface pa-5 rounded-xl border-thin mb-6 elevation-0 relative overflow-hidden">
    ...
    <v-snackbar v-model="snackbar" :color="snackbarColor" location="top" rounded="pill">
        <v-icon start>mdi-check-circle</v-icon> {{ snackbarText }}
    </v-snackbar>
  </v-container>
</template>
  ...
  methods: {
    ...
    async updateStock() {
      ...
      try {
        ...
        this.snackbarText = 'Stock updated successfully'
        ...
        console.error(e)
        this.snackbarText = 'Failed to update stock'
